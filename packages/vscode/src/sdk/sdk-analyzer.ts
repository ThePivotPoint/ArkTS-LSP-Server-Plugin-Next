...
export class SdkAnalyzer {
  ...
  async toOhosClientOptions(force: boolean = false, tsdk?: string): Promise<OhosClientOptions> {
    ...
    // Force load lib.es5.d.ts and lib.decorators.legacy.d.ts if tsdk is provided (For ArkTS)
    const libES5DTS = tsdk ? path.join(tsdk, 'lib.es5.d.ts') : undefined
    const libDecoratorsLegacyDTS = tsdk ? path.join(tsdk, 'lib.decorators.legacy.d.ts') : undefined
